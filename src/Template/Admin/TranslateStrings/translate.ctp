...
<nav class="actions col-sm-4 col-xs-12">
	<ul class="side-nav nav nav-pills nav-stacked">
		...
		<li><?= $this->Html->link(__('List Translate Strings'), ['controller' => 'TranslateStrings', 'action' => 'index', '?' => $this->request->query]) ?></li>
		<li><?php echo $this->Html->link(__('Edit Translate String'), ['action'=>'edit', $translateString['id']]);?></li>
	</ul>
</nav>
...
<?php echo $this->Form->create($translateString);?>
	<fieldset>
		<legend><?php echo __('Translate This String');?></legend>
	...
	if ($translateString->plural) {
		foreach ($translateLanguages as $translateLanguage) {
			...
			echo $this->Form->input('content_'.$key, ['type'=>'text', 'label'=> __('Singular'). ' ' . $translateLanguage['iso2'], 'rel'=>$key]);
		}

		foreach ($translateLanguages as $translateLanguage) {
			$key = $translateLanguage['iso2'];
			//TODO add plural 3 to 6 if necessary
			echo $this->Form->input('plural_2_'.$key, ['type'=>'text', 'label'=>__('Plural') . ' ' . $translateLanguage['iso2'], 'rel' => 'p' . $key]);
...
<?php echo $this->Form->button(__('Save'), ['name' => 'save', 'value' => 'Task']);?>

<?php echo $this->Form->button(__('Save').' + '.__('Next'), ['name' => 'next', 'value' => 'Task', 'class' => 'btn btn-success']);?>
